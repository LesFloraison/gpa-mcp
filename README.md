...
- `open_capture`: opens a `.gpa_frame` with `FrameAnalyzer.exe`, runs the packaged GPA export plugin, and caches a JSON snapshot under the runtime state directory.
- `find_events`: searches API/event calls by name, API `eid` range, and grouping marker text. Results include the 1-based GPA UI `event` number when available.
- `resolve_event`: maps a 1-based GPA UI `event` number to the underlying API `eid` used by the detailed inspection tools.
...
gpa-mcp run-local-json resolve_event --params '{"event":1292}'
...
- Runtime state is written under `%LOCALAPPDATA%\gpa-mcp\.state` on Windows, or `$HOME/.local/share/gpa-mcp/.state` when `%LOCALAPPDATA%` is unavailable. Set `GPA_MCP_WORK_ROOT` to override the root directory.
  For example, `GPA_MCP_WORK_ROOT=D:\CDX\gpa-mcp` writes state under `D:\CDX\gpa-mcp\.state`.
  The state directory includes:
  - frame exports in `.state/exports`
  - texture export requests/results in `.state/requests`
  - texture outputs in `.state/textures`
- GPA UI event numbers are 1-based positions in GPA's event-call list. MCP detail tools use `eid`, the underlying API call id. Use `resolve_event` when starting from a number shown in the GPA UI.
